...
#	if($flag_ratio==1){
#	 	print "$chr:$rclip_pos-$lclip_pos\t$repeat_type;$ref_repeat\tpass\n";
#	}else{
		if(($ref_repeat eq "nan") || ($repeat_type eq "nan")){
			if(/disc_only/){
				print "$chr:$rclip_pos-$lclip_pos\t$repeat_type;$ref_repeat\tpass\tdisc_only\n";
			}else{
				 print "$chr:$rclip_pos-$lclip_pos\t$repeat_type;$ref_repeat\tpass\n";
			}
		}else{
			if(/disc_only/){
				print "$chr:$rclip_pos-$lclip_pos\t$repeat_type;$ref_repeat\tfiltered\tdisc_only\n";
			}else{
				print "$chr:$rclip_pos-$lclip_pos\t$repeat_type;$ref_repeat\tfiltered\n";
			}
		}
#	}
}
